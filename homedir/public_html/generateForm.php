...
{	//var_dump($_POST); exit;
	...
	if(round($_POST['fare'])!=round($_SESSION['rns_fare'])) { 
		header("location:SeatLayout.php"); 
	} else {
	//echo round($_POST['fare'])."==".round($_SESSION['rns_fare']);exit;
	$json=array();
	$user_name=array();
	$user_gender=array();
	$user_age=array();
...
//$order_id = getdata("svr_api_orders_temp", "ba_id", "ba_unique_id = '".$_SESSION[$svr.'busbook']."'");
$qry = query("select ba_id from svr_api_orders_temp where ba_unique_id='".$_SESSION[$svr.'busbook']."'");
$ord_id = mysql_fetch_array($qry);

$order_id = $ord_id['ba_id'];

		//ba_trip_id='".$chosenbusid."' &&
...
$_SESSION[$svr.'order_id'] = mysqli_insert_id($conn);
 $order_id = $_SESSION[$svr.'order_id'];  
	...
	if(!empty($_SESSION[$svra.'ag_id'])){
		...
		header("location:payticket.php?order_id=".$_SESSION['order_id']."&horder_id=".$_SESSION['horder_id']);
	}
	}
} 
...
$fare=$_SESSION['fare'];
if(!empty($_SESSION['actual_hfare']) && !empty($_SESSION['hpersons'] )&& ($_SESSION['sourceList']== 6) && ($_SESSION['destinationList']== 635)){
$hot_fare=$_SESSION['actual_hfare'];
...
echo "<input type='hidden' name='fare' id='fare' class='btnclass' value='".($fare+$hot_fare)."' />";
echo "<input type='hidden' name='canPolicy' id='canPolicy' class='btnclass' value='".$canPolicy."' />";
echo "<label>&nbsp;</label></td>";
echo "<td id='submit_button'><input type='submit' value='Proceed to Pay (Rs.".($fare+$hot_fare).")' class='submit' onClick='return validation()' /></td>";
