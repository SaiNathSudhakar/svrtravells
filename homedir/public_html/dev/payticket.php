...
{	
	...
	$query = query("select ba_id, ba_trip_id, ba_source, ba_source_name, ba_destination_name, ba_destination, ba_total_fare, ba_address, ba_email, ba_mobile, ba_journey_date, ba_name from svr_api_orders_temp as ba
	...
	$success_url = 'http://www.svrtravelsindia.com/confirm-ticket.php?status=1&orderid='.$row['ba_id'];
	$failure_url = 'http://www.svrtravelsindia.com/confirm-ticket.php?status=2&orderid='.$row['ba_id'];
} 

if(!empty($_GET['horder_id']))
{	
	$horder_id = $_GET['horder_id'];
	$hquery = query("select bot_amount from svr_book_order_temp where bot_id = '".$horder_id."'");
	$hrow = fetch_array($hquery);
	$hotal_amt=$hrow['bot_amount'];
}	
?>
<body onLoad="check();">
    <!--<h2>PayU Form</h2>-->
    ...
    <form action="<?php echo $action; ?>" method="post" name="payuForm">
      ...
      <table style="display:none">
        ...
        <tr>
          <td>Amount: </td>
          <td><input name="amount" value="<?=(!empty($row['ba_total_fare'])) ? ($row['ba_total_fare']+$hotal_amt) : ((!empty($_POST['amount'])) ? $_POST['amount'] : ''); ?>" /></td>
          <td>First Name: </td>
