...
{	
	...
	while($row_logo=fetch_array($qur_logo))
	{
...
function get_st_office_emp($emp)
{
	...
	$j=0; while($row1=fetch_array($qur1)){ $j++;
		if($emp==$row1['emp_id']) 
...
$api_order_status = array('0' => 'Pending', '1' => 'Refund', '2' => 'Success', '3' => 'Failure');
$bank = array('' => '----', '1' => 'ICICI', '2' => 'SBI');
$req_status = array('0' => 'Pending', '1' => 'Accepted', '2' =>'Rejected'); //, '4' => 'Recharge'
...
function get_fare_months($loc = '')
{
	...
	while($row = fetch_array($fq)) $fare_mon[] = $row;
	return $fare_mon;
}
...
{	
	...
	//if($fcq_count > 0) { while($row = fetch_array($fcq)) $fare_cats[] = $row; }
...
{	
	...
	while($row_pkg = fetch_array($qur_pkg)) { 
