...
if($_SERVER['REQUEST_METHOD']=="POST")
{
	...
	if($transc_id == 0)
	{
	  ...
	  {
		if(empty($_GET['id'])){
			query("insert into svr_agent_deposits(ad_id, ad_ag_id, ad_amount, ad_transaction, ad_bank, ad_drawn, ad_chq_issue_date, ad_dd_no, ad_acc_holder, ad_attach_slip, ad_type, ad_status, ad_addeddate) values('','".$_SESSION[$svra.'ag_id']."','".$_POST['amount']."','".$_POST['transaction']."','".$_POST['bank']."','".$cq_drawn."','".$cq_datepicker."','".$cq_dd."','".$cq_holder."','".basename($file_upload)."','".$_POST['source']."',1,'".$now_time."')");
		} else {
			query("update svr_agent_deposits set ad_amount='".$_POST['amount']."', ad_transaction='".$_POST['transaction']."', ad_bank='".$_POST['bank']."', ad_drawn='".$cq_drawn."', ad_chq_issue_date='".$cq_datepicker."', ad_dd_no='".$_POST['cq_dd']."', ad_acc_holder='".$cq_holder."', ad_attach_slip='".basename($file_upload)."', ad_type='".$_POST['source']."' where ad_id=".$_GET['id']." and ad_ag_id = ".$_SESSION[$svra.'ag_id']);
		}
...
$sam = query("select ad_id, ad_ag_id, ad_amount, ad_transaction, ad_bank, ad_drawn, ad_chq_issue_date, ad_dd_no, ad_acc_holder, ad_attach_slip, ad_type, cnt_content from svr_agent_deposits
