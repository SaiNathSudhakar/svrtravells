...
<?php 
  ...
  echo "<thead><tr>";
  echo "<th class='sortable' style='text-align:left;'><span>Travels</span></th>";
  echo "<th class='sortable' style='text-align:left;'><span>Bus Type</span></th>";
  echo "<th class='sortable' style='text-align:center;'><span>Departure Time</span></th>";
  echo "<th class='sortable' style='text-align:center;'><span>Arrival Time</span></th>";
  echo "<th class='sortable' style='text-align:center;'><span>Seats</span></th>";
  echo "<th class='sortable' style='text-align:center;'><span>Fares</span></th>";
  echo "<th style='text-align:center;'><span>Seat Layout</span></th>";
  echo "<th style='text-align:center;'>Info</th>";
  echo "</tr></thead>";
?>
  </table></div>
<?
...
foreach ($result2 as $key => $values) { 
    ...
    {   
        ...
        foreach ($values as $k => $v) {
            ...
            foreach ($v as $k1 => $v1) 
            {	
                if(!strcmp($k1,'arrivalTime')||!strcmp($k1,'availableSeats')||!strcmp($k1,'busType')||!strcmp($k1,'cancellationPolicy')||!strcmp($k1,'departureTime')||!strcmp($k1, 'travels')||!strcmp($k1,'fares')||!strcmp($k1,'id')||!strcmp($k1, 'boardingTimes') || !strcmp($k1, 'droppingTimes'))
                {
                   $countkeys++;
                   if (!strcmp($k1,'departureTime')) {
                   ...
                   else if (!strcmp($k1,'cancellationPolicy')) {
				   		$canPol = $v1;
                     	$hold= canPolicy($v1,$countrows,$fareArr);
                   } 
				   else if(!strcmp($k1,'fares')) {
                		$countkeys++;
                  		$columns[$i++]=$k1;
				   ...
				   else if(!strcmp($k1,'busType')){
                  		$bt=$v1;
                   }
                   else if(!strcmp($k1,'travels')){
                        //echo "<td style='text-align:left;'>".$v1."</td>";
                    	$columns[$i++]=$k1; 
						$travels = $v1;
                   }
                   else if (!strcmp($k1,'availableSeats')) {
			 ...
			 echo "<td style='text-align:left;'>".$travels."</td>";
			 echo "<td style='text-align:left;'>".$bt."</td>";
			 echo "<td style='text-align:center;'>".$dpt."</td>";
			 echo "<td style='text-align:center;'>".$art."</td>";
			 echo "<td style='text-align:center;'>".$as."</td>";
			 echo "<td style='text-align:left;'>".$fares."</td>";
    ...
    else
    {
		...
		else // if only one service for the particular route
		{
			...
			{
				if(!strcmp($k1,'arrivalTime')||!strcmp($k1,'availableSeats')||!strcmp($k1,'busType')||!strcmp($k1,'cancellationPolicy')||!strcmp($k1,'departureTime')||!strcmp($k1,'id')||!strcmp($k1, 'travels')||!strcmp($k1,'fares')||!strcmp($k1,'id')||!strcmp($k1, 'boardingTimes') || !strcmp($k1, 'droppingTimes'))
				{
			   		$countkeys++;
			   		if (!strcmp($k1,'departureTime')) {
						$timtim = date('h:i A', mktime(0,$v1));//getTime($v1);
					...
					else if(!strcmp($k1,'id')) {    
						$rowbusid[$countrows]=$v1;
						$storebusid=$v1;
					}
					else if (!strcmp($k1,'boardingTimes')) {
						$boarding = boardingpoints($v1);
					}
					else if (!strcmp($k1,'droppingTimes')) {
						$dropping = droppingpoints($v1);
					}
					else if(!strcmp($k1,'busType')){
                  		$bt=$v1;
