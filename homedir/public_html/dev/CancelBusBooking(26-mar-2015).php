...
if(!empty($_GET['success'])){
	if($_GET['success'] == 'true'){ 
		$close = '<table align="center" height="100%"><tr><td align="center" valign="middle"><table><tr><td>Operation Successful.</td></tr></table></td></tr></table>'; 
	} else {
	...
	echo '<body onload="setTimeout (\'parent.$.fancybox.close ()\',1000); ">'.$close.'</body>';
}
...
/*if($partcan){
	$seats = array("4", "3", "5");
	$charges = array("0", "0", "0");
	$farez = array("300.00", "300.00", "300.00");
...
{	
	...
	if($msg == '') {
		...
		if($partcan)
			//$cancel_type = ((sizeof($sel_seats) == sizeof($old_seats)) || sizeof($old_seats) == 1) ? 4 : 5;
		...
		$cancel_it = cancelTicket($makeCancellationRequest);
		...
		if($flag == 1)
		{
			query("update svr_api_orders set ba_order_status = '".$cancel_type."', ba_seat_status = '".$new_seat_status."', ba_cancel_charges = '".$new_cancel_charges."', ba_refund_amount = '".$new_refund_amount."', ba_cancel_dates = '".$new_cancel_dates."' where ba_id = ".$fetch['ba_id']);
			...
			$data['to_email'] = $email;
			send_email($data);
			header('location:CancelBusBooking.php?success=true');
		} else{
			header('location:CancelBusBooking.php?success=false');
		}
	}
...
<form method='post' name='cancel_booking_2' id='cancel_booking_2'>
    ...
    <table border="0" class="mt10" width="100%">
     ...
     <table width="100%" border="0" class="mt10" cellpadding="2" cellspacing="2">
      ...
      <tr>
        <td colspan="6" align="center"><input name="cancel_ticket" id="cancel_ticket" type="submit" class="submit"  value="Cancel Ticket" /></td>
      </tr>
    </table></td>
      </tr>
      <? } else {?>
