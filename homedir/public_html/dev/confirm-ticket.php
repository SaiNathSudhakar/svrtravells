...
{	
	...
	query("update svr_book_order_temp set bot_request_status = '".$flag."' where bot_id = '".$_SESSION['horderid']."'");
	
	query("insert into svr_book_order (ord_order_id, ord_tmp_id, ord_journey_date, ord_return_date, ord_pkg_id, ord_amount, ord_cust_id, ord_type, ord_no_of_persons, ord_seat_number, ord_ag_id, ord_added_by, ord_status, ord_added_date)
		
		select bot_order_id, bot_id, bot_journey_date, bot_return_date, bot_pkg_id, bot_amount, bot_cust_id, bot_type, bot_no_of_persons, bot_seat_number, bot_ag_id, ord_added_by, bot_status, bot_added_date from svr_book_order_temp where bot_id = '".$_SESSION['horder_id']."')");
...
$fkid = mysqli_insert_id($conn);
	...
	if($fetch['ba_order_status'] == 2)
	{
		...
		if($hdetails_count>0){
			...
			//$hmailheader .= "Bcc: janardhan@svrtravelsindia.com"."\r\n";
