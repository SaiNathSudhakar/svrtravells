...
if(!empty($_GET['lid']) && $row_loc['tloc_banner_image'] != '') { 
	...
	$path = $site_url.'uploads/destination_locations/'.$row_loc['tloc_ref_no'].'/';
...
if((!empty($fixed_fares) || !empty($tour_fares)) && !empty($fare_cats)) { ?>	 
	<div style="width:690px; margin-top:10px;">
       	<div class="fl mr40"><input name="book" type="button" class="submit-btn" value="Book Now" onclick="validate('<?=$row_loc['cat_id_fk']?>');"/></div>
...
//while($row_plcs=mysql_fetch_array($qur_plcs)){
if($place_names != ''){
...
<div class="box_boder">
	<div class="image">
		<? 
		$thumb = $site_url."uploads/places_covered/".$place_ref_no[$key]."/".$place_thumbs[$key];
		//$place_thumb = (@getimagesize($thumb)) ? $thumb : $default_thumb; ?>
		<img src="<?=$thumb;?>" title="<?=$place_name;?>" alt="<?=$place_name;?>" width="120" height="72" align="absmiddle" />
	</div>
	<div class="txt">
		<h1><?=$place_name;?></h1>
		<p><?=$place_small_desc[$key];?></p>
	</div>
	<div class="clear"></div>
</div>
<? } }
