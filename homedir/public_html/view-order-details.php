...
$q = "select ord_ag_id, ord_journey_date, ord_return_date, ord_cust_id, floc_name, floc_id, ord_id, ord_order_id, ord_journey_date, ord_return_date, ord_pkg_id, ord_amount, ord_type, ord_acc_type, ord_room_type, ord_vehicle_type, ord_fc_qty, ord_tot_adult, ord_tot_child, ord_no_of_persons, ord_seat_number, ord_pickup_from, ord_pickup_place, ord_pickup_place_detail, ord_pickup_time, ord_drop_at, ord_drop_place, ord_drop_place_detail, ord_drop_time, ord_emergency_number, ord_comments, ord_total_amount, ord_request_status, tloc_name, tloc_code, tloc_room_type, tloc_type, cust_fname, cust_lname, cust_email, cust_mobile, cust_address_1, cust_address_2, cust_city, cust_state, cust_country, cust_pincode, group_concat(fc.fc_id) as fc_ids, ord_fc_qty as fc_qtys, veh.vp_name as vehicle from svr_book_order as ord
		...
		left join svr_vehicles_pax as veh on veh.vp_id = ord.ord_room_type
			left join svr_customers as cust on cust.cust_id = ord.ord_cust_id
				left join svr_to_locations as tloc on tloc.tloc_id = ord.ord_tloc_id
					left join svr_from_locations as floc on floc.floc_id = tloc.tloc_floc_id where ord_id='".$_GET['id']."'";
$qur = query($q);
$row = fetch_array($qur);

if(!empty($_SESSION[$svra.'ag_id'])) { 
	if($row['ord_ag_id'] != 0 && $row['ord_ag_id'] != $_SESSION[$svra.'ag_id'])
		header('location:agent-account.php');
} else { 
	if($row['ord_cust_id'] != 0 && $row['ord_cust_id'] != $_SESSION[$svr.'cust_id'])
  ...
  <tr>
    <td colspan="2"><table width="100%" border="0" align="center" cellpadding="6" cellspacing="1" class="curve-border">
	  <tr>
		...
		<h2><?=($row['ord_type'] == 1)?'Fixed Departure':'Tour Package';?></h2>
		<?=$product_info;?></td>
      </tr>
	  <tr>
