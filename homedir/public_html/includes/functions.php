...

{
	...
	$res=query("SELECT $field FROM $tble where $whr ");

	$row=fetch_array($res, MYSQL_BOTH);
...

{
	...
	$res=query("SELECT * FROM $tble where $whr ");
...

{
	...
	$qur_logo=query("select * from `svr_logos` where logo_status=1");

	while($row_logo=fetch_assoc($qur_logo))
...

{
	...
	$qur1=query("select e.emp_id,e.emp_name,e.emp_lastname from `tm_emp` as e where e.emp_status=1");

	$j=0; while($row1=fetch_assoc($qur1)){ $j++;
...
function galleryImagesHome($images = '', $path = '')

{

	global $default_gallery_images;
	global $default_path;
	//$path ="http://localhost//svrtravelsindia/homedir/public_html/uploads/default/";
	...
	<div class="slider-frame">
		<div class="sliderImages">
			...
			<? foreach($images as $image){?>
				<li>
					<a href="#nogo" class="mySliderImages">
						<img src="<?=$path.$image?>" width="100%" height="600px" class="slimg" style="display:none" /></a></li>
...
{
	$fare_mon[] =0;
	$fq = query("select fr_id, fr_from_date, fr_to_date, fr_group_id from svr_fares where fr_status = 1 and fr_loc_id = '".$loc."' and fr_to_date > CURDATE() group by fr_group_id order by fr_from_date");
	while($row = fetch_assoc($fq)){
		$fare_mon[] = $row;
	}
...

{
	...
	$fcq = query("select concat(veh.vp_name, ' ', pax.vp_name, ' PAX') as vehicle from svr_fares as fr
	...
	$fare_cats = array(); $fcq_count = num_rows($fcq);

	//if($fcq_count > 0) { while($row = fetch_assoc($fcq)) $fare_cats[] = $row; }
...

{
...

	{
		...
		$prq = query("select fr_id, fr_type, fr_fc_id, fc_orderby, fr_acc_type, fr_room_type, fr_data, fr_group_id, fc_name from svr_fares as fr
		...
		$prq = query("select fr_id, fr_type, fr_fc_id, fc_orderby, fr_acc_type, fr_room_type, fr_data, fr_group_id, concat(veh.vp_name, ' ', pax.vp_name, ' PAX') as vehicle from svr_fares as fr
...

{
	...
	<div class="mining_tbl">
		...
		<? foreach($fare_cats as $fare_cat) {  $veh_names[$fare_cat]=$farecatnames[$fare_cat]; }

			$keys = array_keys($veh_names);

			array_multisort(preg_replace('~-\d+|\D+~','',$veh_names),$veh_names,$keys);

			$veh_names_arr = array_combine($keys,$veh_names);

			$j = 0; foreach($veh_names_arr as $veh_key => $veh_name) { $j++; ?>

		<ul style="width:auto; <?=($j == sizeof($fare_cats)) ? 'border:1px dotted #C9C9C9;' : ''?>">

			<li class="title"><?=$veh_name;?></li>

			<? foreach($acc as $ac) { ?>

			<li><span class="rupee">&#x20B9;</span> <?=((!empty($tour_fares[$veh_key][$ac])) ? $tour_fares[$veh_key][$ac] : 0).'/-';?></li>
...
<? }}
function tour_fares_table_sort($fare_cats, $farecatnames, $acc, $tour_fares)

{
	...
	<div class="mining_tbl">
        ...
        <?  foreach($fare_cats as $fare_cat) {  $veh_names[]=$farecatnames[$fare_cat]; }?>

			<? array_multisort(preg_replace('~\D+~','',$veh_names),$veh_names);

		$j = 0;	foreach($veh_names as $veh_name) {$j++;?>
		...
		<? //$j = 0; foreach($fare_cats as $fare_cat) { $j++; ?>
			...
			<li><span class="rupee">&#x20B9;</span> <?=((!empty($tour_fares[$fare_cats[$j-1]][$ac])) ? $tour_fares[$fare_cats[$j-1]][$ac] : 0).'/-';?></li>
...
function hotel_fares_table($hloc_ids, $hloc_names, $hacc, $hotel_names, $hotel_ids)

{
	...
	if(!empty($hotel_names) && !empty($hloc_ids)){
	...
	<div class="mining_tbl">
		...
		<ul>
			...
			<? foreach($hacc as $hac){ ?><li><?=$room_type[$hac];?></li><? }?>

		</ul>

		<? $j = 0; foreach($hloc_ids as $hloc_id) { $j++; ?>

		<ul style="width:auto; <?=($j == sizeof($hloc_ids)) ? 'border:1px dotted #C9C9C9;' : ''?>">

			<li class="title"><?=$hloc_names[$hloc_id];?></li>

            <?
			 foreach($hacc as $hac){
			?>

            <li><? $hids=$hotel_ids[$hloc_id][$hac]; if($hids!="") { $hids_count=count(explode(",",$hids)); if($hids_count==1) { echo $hotel_names[$hids];} else { $hname=""; foreach(explode(",",$hids) as $hid) { $hname.=$hotel_names[$hid].", "; }echo rtrim($hname,", ");}} else { echo "&nbsp;"; } ?></li>
...

{
	...
	$q = query("select max(pax.vp_min) as vp_min, max(pax.vp_max) as vp_max from svr_fares as fr
...

{
	...
	$q= query("select ch_charges from svr_charges where ch_type = 2 and ch_room_type = ".$room_type." and FIND_IN_SET('".$loc."', ch_locations)");
...

{
	...
	$q = query("select ch_charges, ch_km from svr_charges where ch_type = 1 and ch_vehicle_id = '".$vehicle."' and FIND_IN_SET('".$loc."', ch_locations)");
echo "select ch_charges, ch_km from svr_charges where ch_type = 1 and ch_vehicle_id = '".$vehicle."' and FIND_IN_SET('".$loc."', ch_locations)";
	$fetch = fetch_assoc($q);
...

{
...

	{
	  	...
	  	$q = query("select fr_data from svr_fares as fr
...

{
	...
	while($row_pkg = fetch_assoc($qur_pkg)) {
...
function send_email($data){
	...
	$mailto = "info@svrtravelsindia.com,sales@svrtravelsindia.com,jani@svrtravelsindia.com";
	$mailheader  = "MIME-Version: 1.0" . "\r\n";
	$mailheader .= "Content-type: text/html; charset=iso-8859-1" . "\r\n";
...
//Home Page
function getImagePath($arr, $row, $folderName) {
	return ($arr[$row]['tourPlace'] != '') ? 'images/home/'.$folderName.'/'.$arr[$row]['tourPlace'] : $default_thumb;
}
function getImageName($arr, $row) {
	return $arr[$row]['locName'];
...
function getBanners() {
	...
	//Navigational
	return	$nqur = query($query);
}
function getFixedDepatureInfo() {
	//Fixed Depature
	$fixedDepature = array (
		array("tourPlace"=>"day11409636656.jpg","locName"=>"Char Dham Yatra","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>3),
		array("tourPlace"=>"day11409636656.jpg","locName"=>"Holy Amarnath Yatra","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>1),
		array("tourPlace"=>"day11409636656.jpg","locName"=>"Do Dham Yatra","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>4),
		array("tourPlace"=>"day11409636656.jpg","locName"=>"Best of North India","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>5),
		array("tourPlace"=>"day11409636656.jpg","locName"=>"Holy Muktinath Yatra","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>8),
		array("tourPlace"=>"day11409636656.jpg","locName"=>"DESTINANTION ANDAMAN","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>128)
	);
	return $fixedDepature;
}
function getInternationalInfo() {
	//International
	$international = array (
	  array("locName"=>"Best Of Srilanka","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>9),
		array("locName"=>"Singapore and Malaysia","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>114),
		array("locName"=>"Dubai & Abu Dhabi","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>120),
		array("locName"=>"Thailand","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>133),
		array("locName"=>"Grand Tour Of Ramayana ","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>132)
	);
	return $international;
}
function getTourPackagesInfo() {
	//Tour Packages
	$tourPackages = array (
	  array("locName"=>"India Tour Packages","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>24),
		array("locName"=>"Srilanka, Nepal and Bhutan","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>22),
		array("locName"=>"LTC / LFC","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>2),
		array("locName"=>"Corporate Tours","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>4),
		array("locName"=>"Corporate Tours","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>28),
		array("locName"=>"Adventurs Trips","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>29),
		array("locName"=>"Student EducationTours ","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>3),
		array("locName"=>"Tours From Hyderabad","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>20),
		array("locName"=>"Tours from Delhi","tourPlace"=>"day11409636656.jpg","days"=>"11Nights/12Days","price"=>"2000","reqMore"=>"req more","id"=>21)
	);
 return $tourPackages;
}
?>
