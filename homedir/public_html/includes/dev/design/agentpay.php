		...
		$mail_body = "
			...
			if($qurcount == 1) {
			  ...
			  $mail_body .= "<tr><td colspan='3'><table border='1' cellpadding='5' cellspacing='0' bordercolor='#999999'>
				...
				$ord_sel = mysql_query("select tloc_name, ord_id, ord_order_id, ord_no_of_persons, ord_tot_adult, ord_tot_child, ord_seat_number, ord_amount, ord_total_amount from svr_book_order as ord
					left join svr_to_locations as loc on loc.tloc_id = ord.ord_tloc_id
						where ord_order_id = '".$row['ord_order_id']."'");
				while($cs_row = mysql_fetch_array($ord_sel)){ 
				   $mail_body.="<tr>
