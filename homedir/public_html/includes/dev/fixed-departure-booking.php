...
{	
	...
	{	
		...
		$q = mysql_query("select bot_id, bot_amount, bot_fc_id, bot_fc_qty, bot_no_of_persons, bot_tot_adult, bot_tot_child, bot_seat_number from svr_book_order_temp where bot_pkg_id='".$pkg_id."' and bot_acc_type = '".$bus_type."' and bot_pickup_place = '".$pickup_place."' and bot_order_id = '".$_SESSION[$svr.'fixed_order_id']."' and bot_added_date > subtime('".$now_time."','1:0:0')");
	...
	{	
		...
		mysql_query("insert into svr_book_order_temp (`bot_id`, `bot_order_id`, `bot_tloc_id`, `bot_journey_date`, `bot_return_date`, `bot_pkg_id`, `bot_amount`, `bot_cust_id`, `bot_type`, `bot_acc_type`, `bot_fc_id`, `bot_fc_qty`, `bot_no_of_persons`, `bot_tot_adult`, `bot_tot_child`, `bot_seat_number`, `bot_pickup_from`, `bot_pickup_place`, `bot_pickup_place_detail`, `bot_pickup_time`, `bot_drop_at`, `bot_drop_place`, `bot_drop_place_detail`, `bot_drop_time`, `bot_request_status`, `bot_status`, `bot_added_by`, `bot_added_date`, `bot_ag_id`)  values ('', '".$_SESSION[$svr.'fixed_order_id']."', '".$to_location."', '".$journey_date."', '".$return_date."', '".$pkg_id."', '".$amount."', '".$cust_id."', '1', '".$bus_type."', '".$fare_cat_ids."', '".$fare_cat_qty."', '".$no_of_persons."', '".$tot_adult."', '".$tot_child."', '".$seat_nos."', '".$pickup_from."', '".$pickup_place."', '".$pickup_detail."', '".$pickup_time."', '".$drop_at."', '".$drop_place."', '".$drop_place_detail."', '".$drop_time."', '0', '1', '".$addedby."', '".$now_time."', '".$agid."')");
		...
		mysql_query("update svr_book_order_temp set `bot_amount` = '".$amount."', `bot_fc_id` = '".$fare_cat_ids."', `bot_fc_qty` = '".$fare_cat_qty."', `bot_no_of_persons` = '".$no_of_persons."', `bot_tot_adult` = '".$tot_adult."', `bot_tot_child` = '".$tot_child."', `bot_seat_number` = '".$seat_nos."' where bot_id = '".$fetch['bot_id']."' and bot_added_date > subtime('".$now_time."', '".$ftime_span."')");
...
{	
	...
	$prq = mysql_query("select loc.cat_id_fk, tloc_id, tloc_type, tloc_floc_id, tloc_transport, tloc_code, tloc_pickup_point, cat_name, tloc_status, tloc_time, tloc_ref_no, tloc_banner_image, loc.subcat_id_fk, subcat_name, tloc_id, tloc_name, tloc_places_covered, tloc_notes, tloc_cost_includes, tloc_cost_excludes, tloc_acc_type, tloc_pickup_place, cnt_content, fc_name, fc_multiple, fc_adult_child, fr_fc_id, fr_acc_type, fr_data, pkg_id, pkg_date, GROUP_CONCAT(pick.pick_id SEPARATOR '|') AS pickup_ids, GROUP_CONCAT(pick.pick_name SEPARATOR '|') AS pickup_points from svr_fares as fr
				...
				left join svr_subcategories as subcat on loc.subcat_id_fk = subcat.subcat_id 
					left join svr_pickup_points as pick on pick.tloc_id_fk = loc.tloc_id and pick.pick_status = 1
						left join svr_packages as pkg on pkg.pkg_to_id = loc.tloc_id and pkg_date = '".db_date($_GET['date'])."'
							where fr_status = 1 and ('".date('Y-m-d', strtotime($_GET['date']))."' between fr_from_date and fr_to_date) and tloc_id=".$_GET['lid']." group by fr_fc_id, fr_acc_type");					
...
//echo $q; exit;
$qur_pkg = mysql_query($q);
$row_pkg = mysql_fetch_array($qur_pkg);
