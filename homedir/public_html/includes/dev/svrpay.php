...
{	
	...
	$query = mysql_query("select bot_order_id, bot_type, bot_total_amount, tloc_name, tloc_code, tloc_type, cust_fname, cust_lname, cust_email, cust_mobile, cust_address_1, cust_address_2, cust_city, cust_state, cust_country, cust_pincode from svr_book_order_temp as bot
	...
	//mysql_query("update svr_book_order_temp set bot_request_status = '2' where bot_order_id = '".$row['bot_order_id']."'");
	...
	$success_url = $site_url.'payment-status.php?status=1&orderid='.$row['bot_order_id'];
	$failure_url = $site_url.'payment-status.php?status=2&orderid='.$row['bot_order_id'];
} ?>
<body onLoad="check();">
    <!--<h2>PayU Form</h2>-->
    <br/>
