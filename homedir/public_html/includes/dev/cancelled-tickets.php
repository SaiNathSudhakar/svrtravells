...
if(!empty($_GET['id']) && !empty($_GET['amount']) && !empty($_GET['orderid']))
{	
	...
	mysql_query("update svr_book_order_temp set bot_request_status = 2 where bot_id = ".$_GET['id']);
	mysql_query("update svr_book_order set ord_request_status = 2, ord_cancel_charges = '".$cancel_charges."', ord_cancel_date = '".$now_time."' $cancel_by where ord_tmp_id = ".$_GET['id']);
	...
	if(!empty($_SESSION[$svra.'ag_id'])){
		//mysql_query("update svr_agents set ag_deposit = (ag_deposit+".$_GET['amount'].") where ag_id = '".$_SESSION[$svra.'ag_id']."'");
	...
	header("location:cancelled-tickets.php");
	...
	$len=10; $start=0;
	$link = "cancelled-tickets.php?a=a";
	if(!empty($_GET['start'])) { $start=$_GET['start']; }

	$q = mysql_query("select ord_id, ord_order_id, ord_total_amount, ord_tmp_id, ord_journey_date, ord_updated_date, ord_cancel_charges, ord_cancel_date, tloc_name, cat_name from svr_book_order as ord
	left join svr_to_locations as tloc on tloc.tloc_id = ord.ord_tloc_id
		left join svr_categories as cat on cat.cat_id = ord.ord_type
	...
	$designFILE="design/cancelled-tickets.php";
