...
<? } if(!empty($_POST['to']) && !empty($_POST['avail_tracker'])) { 
	...
	while($row_pkg = mysql_fetch_array($qur_pkg)) $avail_dates[] = date('d-m-Y',strtotime($row_pkg['pkg_date']));
...
<? } if(!empty($_POST['get_fares']) && $_POST['get_fares'] == 2) { 
	...
	$loc = $_POST['loc']; $room_type = $_POST['room_type'];	$vehicle = $_POST['vehicle'];	
	$pax = $_POST['pax']; $min_pax = $_POST['min_pax'];
	$nights = $_POST['nights']; $days = $_POST['days'];  $new_pax = $_POST['new_pax']; 
	
	$data = get_additional_fares($loc, $room_type, $vehicle, $pax, $min_pax, $nights, $days, $new_pax);
 ...
 <? } if(!empty($_POST['get_fares']) && $_POST['get_fares'] == 3) { 
	...
	$data = get_fares_childbed($loc, $room_type, $vehicle, $pax, $min_pax, $nights, $days, $new_pax, $childbed_pax, $childnobed_pax);
...
<? } if(!empty($_POST['get_fares']) && $_POST['get_fares'] == 4) { 
	...
	$data = get_fares_childnobed($loc, $room_type, $vehicle, $pax, $min_pax, $nights, $days, $new_pax, $childbed_pax, $childnobed_pax, $get_fares);
...
<? } if(!empty($_POST['email']) && $_POST['tour_email'] == 1) {
	  ...
	  $q = mysql_query("select cust_password, cust_id, cust_title, cust_fname, cust_lname, cust_email, cust_address_1, cust_mobile, cust_city, cust_state, cust_country from svr_customers where cust_email = '".$_POST['email']."'");
	  $count = mysql_num_rows($q); $row = mysql_fetch_assoc($q);
	...
	$q = mysql_query("select pick_time, pick_note from svr_pickup_points where pick_id = '".$_POST['pick_id']."'");
	...
	$q = mysql_query("select ba_source_name, ba_destination_name, ba_travels_name, ba_total_fare, ba_id, ba_ag_id, ba_journey_date, ba_order_status from svr_api_orders where ba_email = '".$_POST['email']."' and ba_ticket_no = '".$_POST['ticket']."'");
	$count = mysql_num_rows($q); $row = mysql_fetch_assoc($q);
