...
if(!empty($_POST['fares_cat']) && !empty($_POST['fare_type'])) { 
	if($_POST['fare_type'] == 1) { 
		$svr_query = mysql_query("select tloc_id, tloc_name, tloc_code from svr_to_locations where cat_id_fk = 1 and tloc_status = 1 order by tloc_orderby");
	?>
	<select name="sel_destination[]" class="sel_destination" multiple style="width:300px;">
	   <?php while($loc=mysql_fetch_array($svr_query)){ ?>
	   <option value="<?=$loc['tloc_id'];?>"><?=ucwords(strtolower($loc['tloc_name'])).' ('.$loc['tloc_code'].')';?></option>
	   <? }?>
	</select>
<? } if($_POST['fare_type'] == 2) { ?>
	<? $svr_subq = mysql_query("select cat_id_fk, subcat_id, subcat_name from svr_subcategories where cat_id_fk = 2 and subcat_status = 1 order by subcat_orderby"); ?>
	<select name="sel_destination[]" class="sel_destination" multiple style="width:300px;">
	   <?php while($sloc=mysql_fetch_array($svr_subq)){ 
			$svr_query = mysql_query("select tloc_id, tloc_name, tloc_code from svr_to_locations where cat_id_fk = 2 and subcat_id_fk = '".$sloc['subcat_id']."' and tloc_status=1 order by tloc_orderby"); if(mysql_num_rows($svr_query) > 0) { ?>
	...
	<? } else if($_POST['fare_type'] == 2){?>
	  <select name="name" id="name" style="width:150px">
		...
		<? $q = mysql_query('select v.v_id, concat(veh.vp_name, " ", pax.vp_name, " PAX") as vehicle from svr_vehicles_with_pax as v
...
<? } if($_POST['fare_type'] == 2) { 
		$subq = mysql_query("select cat_id_fk, subcat_id, subcat_name from svr_subcategories where cat_id_fk=2 and subcat_status=1 order by subcat_orderby");
	?>
	<select name="sel_subcategory" id="sel_subcategory" style="width:300px;">
       <option value="">Select Tour Package</option>
       <?php while($sub=mysql_fetch_array($subq)){ ?>
	...
	$svr_query = mysql_query("select tloc_id, tloc_name, tloc_code from svr_to_locations where subcat_id_fk = ".$_POST['subcat']." and tloc_status=1 order by tloc_orderby desc");?>
	<select name="sel_destination" id="sel_destination" style="width:300px;">
      <option value="">--Select Location--</option>
	   <?php while($loc=mysql_fetch_array($svr_query)){ ?>
	   <option value="<?=$loc['tloc_id'];?>"><?=ucwords(strtolower($loc['tloc_name'])).' ('.$loc['tloc_code'].')';?></option>
...
<? } if(!empty($_POST['fares']) && !empty($_POST['dest'])) {	
	$ac_query = mysql_query("select cat_id_fk, tloc_acc_type, tloc_room_type from svr_to_locations where tloc_id = ".$_POST['dest']);
	$ac = mysql_fetch_array($ac_query);
...
<table width="100%" align="left"><tr>
  ...
  <? 
  	...
  	$fc_query = mysql_query("select fc_type, fc_id, fc_name, concat(veh.vp_name, ' ', pax.vp_name, ' PAX') as vehicle from svr_fare_category  as fc
	...
	<? $i = 0;
	  while($fare_cat = mysql_fetch_array($fc_query)){?>
	  <tr>
...
{	
	if($_POST['type'] == 1){ 
		$svr_query = mysql_query("select tloc_id, tloc_name, tloc_code from svr_to_locations where cat_id_fk = 1 and tloc_status=1 order by tloc_orderby desc"); ?>
...
<? } if($_POST['type'] == 2){ ?>
		...
		<select name="sel_destination[]" id="sel_destination" multiple style="width:300px;" class="package">
		   <?php while($sloc=mysql_fetch_array($svr_subq)){ 
		   		$svr_query = mysql_query("select tloc_id, tloc_name, tloc_code from svr_to_locations where cat_id_fk = 2 and subcat_id_fk = '".$sloc['subcat_id']."' and tloc_status=1 order by tloc_orderby desc"); if(mysql_num_rows($svr_query) > 0){?>
				<optgroup label="<?=$sloc['subcat_name']?>">
				<? while($loc = mysql_fetch_array($svr_query)){?>
		   			<option value="<?=$loc['tloc_id'];?>">&nbsp;&raquo;&nbsp;<?=ucwords(strtolower($loc['tloc_name'])).' ('.$loc['tloc_code'].')';?></option>
		   <? }}?></optgroup> <? }?>
...
{	
	$svr_query = mysql_query("select floc_id, floc_name from svr_from_locations as floc
	...
	<select name="sel_from_loc" id="sel_from_loc">
	   ...
	   <?php while($row = mysql_fetch_array($svr_query)) { ?>
...
<? } else if(!empty($_POST['package']) && $_POST['package'] == 2 && !empty($_POST['type']) && !empty($_POST['from_loc'])) {	
	$svr_query = mysql_query("select group_concat(tloc.tloc_id order by tloc_orderby desc) as to_ids, group_concat(tloc.tloc_name order by tloc_orderby desc) as to_locations from svr_tour_locations as trl 
	...
	$row = mysql_fetch_array($svr_query);
...
<? if(!empty($_POST['package']) && $_POST['package'] == 3  && !empty($_POST['dest'])) {	
	//&& !empty($_POST['type'])
	$ac_query = mysql_query("select cat_id_fk, tloc_acc_type, tloc_room_type from svr_to_locations where cat_id_fk = 1 and tloc_id = ".$_POST['dest']);
	$ac = mysql_fetch_array($ac_query);
	...
	<table width="100%" align="left">
	  ...
	  <tr>
		...
		<? foreach($ac_types as $key => $ac_type){ ?>
		  <td><input type="text" name="seats[<?=$key?>]"></td>
		<? }?>
	  </tr>
	 </table>		
<? }
...
if(!empty($_POST['cat']) && !empty($_POST['to_loc']) && $_POST['to_loc'] == 1){
	$svr_query = mysql_query("select subcat_id, subcat_name from svr_subcategories where subcat_status = 1 and cat_id_fk = '".$_POST['cat']."' order by subcat_orderby"); ?>
...
<? }if(!empty($_POST['subcat']) && !empty($_POST['to_loc']) && $_POST['to_loc'] == 2){
	$svr_query = mysql_query("select subsubcat_id, subsubcat_name from svr_subsubcategories where subsubcat_status = 1 and subcat_id_fk = '".$_POST['subcat']."'"); ?>
