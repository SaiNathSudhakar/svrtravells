...
$qur=mysql_query("select ag_dep.*,ag.ag_fname, ag.ag_mobile, ag.ag_unique_id from svr_agent_deposits as ag_dep
	left join svr_agents as ag  on ag_dep.ad_ag_id=ag.ag_id where ad_id='".$_GET['dep_id']."'");
  ...
  <tr>
    <td><table width="100%" border="0" align="center" cellpadding="6" cellspacing="1" bgcolor="#E7E7E7" class="curve-border">
      ...
      <tr>
        ...
        <td valign="top" bgcolor="#F3F3F3"><?=$row['ad_amount'];?></td>
      </tr>
	  ...
	  <? }if(!empty($row['ad_chq_issue_date']) && $row['ad_chq_issue_date']!= '0000-00-00 00:00:00' && $row['ad_chq_issue_date']!= '1970-01-01 05:30:30'){?>
	   <tr>
	  ...
	  <tr>
        ...
        <td valign="top" bgcolor="#F3F3F3"><a href="<?=$site_url.'uploads/agents/'.$row['ag_unique_id'].'/'.basename($row['ad_attach_slip']);?>" style="text-decoration:none;"><?=$row['ad_attach_slip'];?></a></td>
      </tr>
	  <? }?>
    </table></td>
  </tr>
