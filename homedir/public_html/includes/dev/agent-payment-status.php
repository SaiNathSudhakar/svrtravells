...
{	
	mysql_query("update svr_agent_deposits_temp set adt_req_status = '".$_GET['status']."' where adt_order_id = '".$_GET['order_id']."'");
	
	if($_GET['status'] == '1')
	{	
		mysql_query("INSERT INTO `svr_agent_deposits` (`ad_order_id`, `ad_ag_id`, `ad_amount`, `ad_req_status`, `ad_type`, `ad_status`, `ad_addeddate`)
		...
		$qur = mysql_query("select ag_fname, ag_lname, ag_mobile, ag_address, ag_email, ag_city, ag_state, ag_country, ag_pincode, ad_ag_id, ad_amount, ad_order_id, ad_id, ad_ag_id, ad_type from svr_agent_deposits as ad
		...
		$amtcount = mysql_num_rows($qur);
		$row = mysql_fetch_array($qur);
		
		$amt = $row['ad_amount'];
		
		$amt = number_format($amt, 2, '.', '');
		mysql_query("update svr_agents set ag_deposit = (ag_deposit + ".$amt.") where ag_id = '".$_SESSION[$svra.'ag_id']."'");
		...
		mysql_query("insert into svr_agent_reports (ar_id, ar_ag_id, ar_ref_id, ar_ord_id, ar_ad_id, ar_transaction_type, ar_opening_bal, ar_amount, ar_commission, ar_net_amount, ar_closing_bal, ar_cre_deb, ar_fk_id, ar_date_time) values( '', '".$_SESSION[$svra.'ag_id']."', '".$ref_id."', '".$_GET['order_id']."', '', '".$transc."', '".$op_bal."', '".$amt."', '', '".$net."', '".$cl_bal."', '1', '".$fkid."', '".$now_time."')");
	...
	$qur=mysql_query("select ag_fname, ag_lname, ag_mobile, ag_address, ag_email, ag_city, ag_state, ag_country, ag_pincode, adt_ag_id, adt_amount, adt_mobile, adt_order_id, adt_id, adt_ag_id, adt_type, adt_req_status from svr_agent_deposits_temp as ad
